...
from lib.models.tracking_engine_1.score_decoder import ScoreDecoder
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv_proj_k = self._build_projection(
            dim_in, dim_out, kernel_size, padding_kv, stride_kv, method, conv_proj_post_norm
        )
...
def get_TE_1_online_model(config, **kwargs):
...
class TE_1_OnlineScore(nn.Module):
    """TE_1_ tracking with score prediction module, whcih jointly perform feature extraction and interaction."""
...
def build_TE_1_cvt_online_score(cfg, settings=None, train=True):
    backbone = get_TE_1_online_model(cfg)  # backbone without positional encoding and attention mask
    box_head = build_box_head(cfg)  # a simple corner head
    score_branch = ScoreDecoder(
        pool_size=4, hidden_dim=cfg.MODEL.HIDDEN_DIM, num_heads=cfg.MODEL.HIDDEN_DIM // 64
    )  # the proposed score prediction module (SPM)
    model = TE_1_OnlineScore(backbone, box_head, score_branch, head_type=cfg.MODEL.HEAD_TYPE)
    ...
    if cfg.MODEL.PRETRAINED_STAGE1 and train:
        try:
            ...
            if is_main_process():
                print("missing keys:", missing_keys)
                print("unexpected keys:", unexpected_keys)
                print("Loading pretrained TE_1 weights done.")
        except:
            print("Warning: Pretrained TE_1 weights are not loaded")
