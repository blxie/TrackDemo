...
from lib.config.tracking_engine_1_online.config import cfg, update_config_from_file
...
def parameters(yaml_name: str, model=None, search_area_scale=None):
    ...
    yaml_file = os.path.join(prj_dir, "experiments/tracking_engine_1_online/%s.yaml" % yaml_name)
    ...
    if model is None:
        ...
        params.checkpoint = os.path.join(save_dir, "model_zoo/%s" % model)
